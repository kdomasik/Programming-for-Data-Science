---
title: "Solutions for ST340 Lab 9"
date: "2019--20"
output: pdf_document
---
...
system.time(resSym <- vbmp(train.X, train.Y, test.X, test.Y, theta,
...
system.time(resMNIST <- vbmp(newXtrain, newYtrain, newXtest, newYtest, theta,
                 list(sKernelType="iprod", bThetaEstimate=FALSE, maxIts=50)))
predError(resMNIST)
```
...
```{r warning=FALSE}
library(e1071)
fit.svm <- svm(newXtrain, newYtrain)
pred.svm <- predict(fit.svm, newXtest)
mean((pred.svm - newYtest)^2)
```
